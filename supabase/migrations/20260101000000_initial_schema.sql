...
CREATE TABLE IF NOT EXISTS public.jobs (
    ...
    fit_tier TEXT NOT NULL DEFAULT 'Unassessed',
...
-- Document limits trigger (max 10 CVs, max 10 Cover Letters per user)
...
AS $$
BEGIN
  IF (SELECT COUNT(*) FROM public.user_cvs WHERE (NEW.user_id IS NOT NULL AND user_id = NEW.user_id) OR (lower(user_email) = lower(NEW.user_email))) >= 10 THEN
...
AS $$
BEGIN
  IF (SELECT COUNT(*) FROM public.user_cover_letters WHERE (NEW.user_id IS NOT NULL AND user_id = NEW.user_id) OR (lower(user_email) = lower(NEW.user_email))) >= 10 THEN
...
CREATE INDEX IF NOT EXISTS idx_user_job_statuses_user_id ON public.user_job_statuses(user_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_user_job_statuses_user_id_job ON public.user_job_statuses(user_id, job_id) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_user_job_statuses_email_job ON public.user_job_statuses (lower(user_email), job_id);
...
CREATE INDEX IF NOT EXISTS idx_user_job_evaluations_user_id ON public.user_job_evaluations(user_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_user_job_evaluations_user_id_job ON public.user_job_evaluations(user_id, job_id) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_user_job_evaluations_email_job ON public.user_job_evaluations (lower(user_email), job_id);

-- ==============================================================================
-- 8. AUTO-POPULATE USER_ID TRIGGER
-- ==============================================================================
...
BEGIN
  ...
  IF NEW.user_id IS NULL AND NEW.user_email IS NOT NULL THEN
    SELECT id INTO NEW.user_id FROM auth.users WHERE lower(email) = lower(NEW.user_email) LIMIT 1;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_set_user_id_profiles ON public.user_profiles;
CREATE TRIGGER trg_set_user_id_profiles
  BEFORE INSERT OR UPDATE ON public.user_profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.set_user_id_from_auth();

DROP TRIGGER IF EXISTS trg_set_user_id_cvs ON public.user_cvs;
CREATE TRIGGER trg_set_user_id_cvs
  BEFORE INSERT OR UPDATE ON public.user_cvs
  FOR EACH ROW
  EXECUTE FUNCTION public.set_user_id_from_auth();

DROP TRIGGER IF EXISTS trg_set_user_id_cover_letters ON public.user_cover_letters;
CREATE TRIGGER trg_set_user_id_cover_letters
  BEFORE INSERT OR UPDATE ON public.user_cover_letters
  FOR EACH ROW
  EXECUTE FUNCTION public.set_user_id_from_auth();

DROP TRIGGER IF EXISTS trg_set_user_id_job_statuses ON public.user_job_statuses;
CREATE TRIGGER trg_set_user_id_job_statuses
  BEFORE INSERT OR UPDATE ON public.user_job_statuses
  FOR EACH ROW
  EXECUTE FUNCTION public.set_user_id_from_auth();

DROP TRIGGER IF EXISTS trg_set_user_id_job_evaluations ON public.user_job_evaluations;
CREATE TRIGGER trg_set_user_id_job_evaluations
  BEFORE INSERT OR UPDATE ON public.user_job_evaluations
  FOR EACH ROW
  EXECUTE FUNCTION public.set_user_id_from_auth();
...
-- 9.2 Shared Catalog (Jobs, Sources, Employers)
...
CREATE POLICY "Service role full access on user_cvs"
  ON public.user_cvs FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

-- 9.5 User Cover Letters
...
CREATE POLICY "Service role full access on user_cover_letters"
  ON public.user_cover_letters FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

-- 9.6 User Job Statuses
...
CREATE POLICY "Service role full access on user_job_statuses"
  ON public.user_job_statuses FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

-- 9.7 User Job Evaluations
...
CREATE POLICY "Service role full access on user_job_evaluations"
  ON public.user_job_evaluations FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

-- ==============================================================================
-- 10. STORAGE BUCKET POLICIES (user-documents)
-- ==============================================================================
DO $$
BEGIN
...
-- Only public jobs and sources tables are published. Private tenant tables are excluded.
-- ==============================================================================
DO $$
BEGIN
  BEGIN
  ...
  BEGIN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.user_cvs;
  EXCEPTION WHEN undefined_object THEN NULL;
  END;

  BEGIN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.user_cover_letters;
  EXCEPTION WHEN undefined_object THEN NULL;
  END;

  BEGIN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.user_job_evaluations;
  EXCEPTION WHEN undefined_object THEN NULL;
  END;
END $$;
...
BEGIN
    ...
    filtered AS (
        ...
        WHERE 
            ...
            AND (
                ...
                (
                    CASE 
                        WHEN p_location = 'dublin' THEN c.location ILIKE '%dublin%'
                        WHEN p_location = 'kildare' THEN (c.location ILIKE '%kildare%' OR c.location ILIKE '%maynooth%' OR c.location ILIKE '%naas%' OR c.location ILIKE '%leixlip%')
                        WHEN p_location = 'laois' THEN (c.location ILIKE '%laois%' OR c.location ILIKE '%portlaoise%' OR c.location ILIKE '%abbeyleix%' OR c.location ILIKE '%mountmellick%' OR c.location ILIKE '%portarlington%')
                        WHEN p_location = 'cork' THEN (c.location ILIKE '%cork%' OR c.location ILIKE '%ringaskiddy%' OR c.location ILIKE '%carrigaline%')
                        WHEN p_location = 'galway' THEN c.location ILIKE '%galway%'
                        WHEN p_location = 'kilkenny' THEN c.location ILIKE '%kilkenny%'
                        WHEN p_location = 'ireland' THEN c.location ILIKE '%ireland%'
                        ELSE c.location = p_location
                    END
                )
            )
            AND (
    ...
    SELECT 
        ...
        COALESCE(
            (SELECT jsonb_agg(row_to_json(r)) FROM paginated r),
            '[]'::jsonb
        )
...
-- 12.3 Permissions: Restrict RPC execution to authenticated users and service_role
REVOKE ALL ON FUNCTION public.get_jobs_page(text, text, text, integer, text, text, text, text, text, integer, integer) FROM PUBLIC, anon;
...
REVOKE ALL ON FUNCTION public.get_overview_metrics(text) FROM PUBLIC, anon;
