...
const CustomTooltip = ({ active, payload, label, onSelectStatus }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div
        className="bg-neutral-900/95 border border-white/10 p-3.5 rounded-2xl shadow-2xl backdrop-blur-xl text-xs space-y-1.5 cursor-pointer"
      ...
      >
        ...
        <div className="text-[11px] text-zinc-400 pt-1 border-t border-zinc-800 flex items-center gap-1 font-medium">
          <span>Click to view {label.toLowerCase()} opportunities →</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-400 mt-0.5">
            Active roles across candidate progression stages (click any stage to filter)
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 pt-2 border-t border-zinc-800">
        {data.map((stage) => (
          <button
            ...
            title={`View ${stage.name} opportunities`}
          >
