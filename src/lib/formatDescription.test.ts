...
describe('formatJobDescription', () => {
  ...
  it('formats dense text by inserting section headers and lists', () => {
    const raw =
      'Acme is looking for an engineer. Key Responsibilities Build reliable services. Requirements 5+ years experience; Strong skills. What We Offer Competitive compensation and perks.';
    const formatted = formatJobDescription(raw);
    expect(formatted).toContain('Key Responsibilities:\nBuild reliable services.');
    expect(formatted).toContain('Requirements:\n5+ years experience;\n• Strong skills.');
    expect(formatted).toContain('What We Offer:\nCompetitive compensation and perks.');
    expect(formatted).not.toContain('compensation:\n');
  });
});
