...
- Candidate Profile & Scoring:
  ...
  - Sub-views: `ProfileGeneralInfo.tsx`, `ProfileTargetPreferences.tsx`, `ProfileQualifications.tsx`, `ProfileDocuments.tsx`
...
- Design System: `src/design-system/`
  - Product-neutral tokens: `src/design-system/tokens.css`
  - Public component API: `src/design-system/index.ts`
  - Usage and extraction contract: `docs/DESIGN_SYSTEM.md`
...
- Use semantic `--ds-*` tokens and `src/design-system/` primitives for new UI.
  Do not introduce raw hex or one-off structural colors in route components.
- Keep `src/design-system/` free of JobPulse domain imports, Supabase, routes,
  and translation hooks so it remains extractable.

## Update Triggers

- If database tables, RLS, or schema change: create a forward migration
  (`npm run db:migration <name>`), test it with a local reset, inspect parity
  (`npm run db:diff`), intentionally push it (`npm run db:push`), and regenerate
  types (`npm run db:types`). Never edit an applied migration.
