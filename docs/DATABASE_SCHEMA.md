...
# 2. Rebuild local Supabase and inspect production parity
make db-reset
npm run db:diff

# 3. Intentionally push the reviewed migration to the linked project
npm run db:push

# 4. Regenerate TypeScript database types
npm run db:types

# 5. Verify code quality gate
npm run check
```
...
> Never edit a migration that has been applied to production; add a forward migration instead.

See [Local Development, Backups & Restore](LOCAL_DEVELOPMENT.md) for the
database-and-Storage backup workflow.
