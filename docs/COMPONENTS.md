...
JobPulse's reusable, product-neutral design system is documented in
[Design System](DESIGN_SYSTEM.md). It contains semantic tokens plus shared
`Button`, `Card`, `PageHeader`, `TextField`, `Select`, and `EmptyState`
primitives. It has no JobPulse data, route, Supabase, or localization
dependencies and can be extracted into a shared package later.

`src/components/ui/` contains JobPulse-specific UI and compatibility modules:

- `button.tsx`: Compatibility export for the design-system `Button`.
