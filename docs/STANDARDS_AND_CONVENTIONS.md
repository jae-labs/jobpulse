...
- **Schema Lifecycle**: When modifying database columns or tables:
  ...
  2. Rebuild locally with `make db-reset` and inspect parity with `npm run db:diff`.
  3. Push the reviewed migration with `npm run db:push`.
  4. Regenerate types with `npm run db:types`.
  5. Ensure `src/types/job.ts` mirrors the database schema.
...
- **Use the public boundary**: New reusable UI must be built in or consume
  `src/design-system/` through `src/design-system/index.ts`. See
  [Design System](DESIGN_SYSTEM.md) for its API and extraction contract.
- **Semantic styling**: Use `--ds-*` tokens or Tailwind `ds-*` utilities for
  structural colors. Do not introduce raw hex values or new `zinc`/one-off
  surface colors in route components.
- **Standard primitives**: Use `PageHeader`, `Card`, `Button`, `TextField`,
  `Select`, and `EmptyState` where their pattern applies. Keep composition,
  copy, translations, data, and routes in the application layer.
- **Portable by design**: `src/design-system/` must not import JobPulse domain
  types, translation hooks, Supabase clients, route state, or query hooks.
